...
def intersection_properties(x1, y1, r1, x2, y2, r2):
    ...
    if abs(r1 - r2) < d < r1 + r2:
        ...
        tangent1 = (intersection1[1] - y1, -(intersection1[0] - x1))  # Perpendicular at intersection1
        tangent2 = (intersection2[1] - y1, -(intersection2[0] - x1))  # Perpendicular at intersection2

        # Normalize tangent vectors
        norm1 = math.sqrt(tangent1[0]**2 + tangent1[1]**2)
        norm2 = math.sqrt(tangent2[0]**2 + tangent2[1]**2)
        tangent1 = (tangent1[0] / norm1, tangent1[1] / norm1)
        tangent2 = (tangent2[0] / norm2, tangent2[1] / norm2)

        # Angle between tangent vectors
        dot_product = tangent1[0] * tangent2[0] + tangent1[1] * tangent2[1]
        angle = math.acos(max(min(dot_product, 1), -1))  # Clamp to avoid numerical issues

        return 2, [tangent1, tangent2], math.degrees(angle)
